 ...
 *   get(i)            i 番目                        O(log n)。木を変えない
 *   prod(l, r)        [l, r) の総積                 O(log n)
 *   all_prod()        全体の総積                    O(1)
 *   to_vec()          列に戻す                      O(n)
 *   clear()           空にする
 *
 *   添字は 0 始まりの半開区間。l >= r の区間は prod なら e()、erase なら
 *   何もしない。
 *   S は avl_value を継承すると x.sz に要素数が入る（op の中で触らなくてよい）。
 ...
 */
template <class S, S (*op)(S, S), S (*e)()> struct avl_segtree {
 private:
  struct node {
  ...
  S prod_(int i, int l, int r) const {
  ...
  void to_vec_(int i, std::vector<S>& out) const {
    if (i < 0) return;
    if (pool[i].lft < 0) {
